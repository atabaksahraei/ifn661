...
namespace healthbook.Model.BL
{
	public class Manager
	{
		...
		static Manager me = null;
		MobileServiceClient client;
		public IMobileServiceSyncTable<Patient> PatientTable;
		public IUIObservable Observer;
		IMobileServiceSyncTable<Doctor> DocTable;
		...
		private Manager ()
		{
			...
			BackgroundThread = Task.Factory.StartNew (() => {
				while(backgroundMode)
				{
					RefreshDataAsync();
					Thread.Sleep(500);
					TraceManager.Trace(CONTEXT, Thread.CurrentThread.ManagedThreadId, "Background Sync");
					if(Observer != null)
					{
						Observer.Redraw();
					}
